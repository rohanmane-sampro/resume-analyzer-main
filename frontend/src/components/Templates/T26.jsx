...
export const T26 = ({ jsonData }) => {
  ...
  return (
    <StyledWrapper cssContent={T26Css}>
      <div className="resume">
        ...
        <div className="main-content">
          ...
          {/* Left Column (Education, Skills, Lang, Certs) */}
              ...
              <div className="section">
                ...
                {jsonData.education.map((edu, index) => (
                  <div key={`edu-${index}`} className="education-item">
                    <div className="education-degree">{edu.degreeName}</div>
                    <div className="education-institution">{edu.institutionName}</div>
                    <div className="education-date">{edu.graduationYear}</div>
                    {edu.currentCGPA && (
              ...
              <div className="section">
                ...
                {hardSkills.length > 0 && (
                  <div className="skill-category">
                    <div className="skill-category-title">Hard Skills</div>
                    <div className="skill-list">
                      {hardSkills.map((skill, index) => (
                        <span key={`hard-${index}`} className="skill-item">{skill.trim()}</span>
                      ))}
                    </div>
                  </div>
                )}
                  ...
                  <div className="skill-category">
                    ...
                    <div className="skill-list">
                      {softSkills.map((skill, index) => (
                        <span key={`soft-${index}`} className="skill-item">{skill.trim()}</span>
                      ))}
                    </div>
                  </div>
                )}
              ...
              <div className="section">
                ...
                {jsonData.certificates.map((cert, index) => (
                  <div key={`cert-${index}`} className="cert-item">
                    <div className="cert-title">{cert.certificateName}</div>
                    <div className="cert-provider">{cert.providerName}</div>
                    {(cert.year || cert.courseDuration) && (
                      <div className="cert-date">{cert.year} {cert.courseDuration ? `• ${cert.courseDuration}` : ''}</div>
                    )}
                  </div>
                ))}
              </div>
          ...
          {/* Right Column (Summary, Exp, Projects) */}
          <div className="right-column">
            ...
            {jsonData.Description?.UserDescription && (
              <div className="section">
                ...
                <div className="section-content" dangerouslySetInnerHTML={{ __html: parseMarkdown(jsonData.Description.UserDescription) }} />
              </div>
            )}
                  ...
                  <div key={`work-${index}`} className="experience-item">
                    <div className="item-header">
                      <div className="item-title">
                        {we.jobTitle}, <span style={{ fontStyle: 'italic', fontWeight: 'normal' }}>{we.companyName}</span>
                      </div>
                      <div className="item-date">
                        {we.WorkDuration} {we.Location ? `| ${we.Location}` : ''}
                      </div>
                    </div>
                  ...
                  <div key={`proj-${index}`} className="project-item">
                    ...
                    {proj.toolsTechUsed && (
                      <div className="project-tech">
                        <strong>Technologies:</strong> {proj.toolsTechUsed}
                      </div>
                    )}
                  </div>
                ))}
