import React from 'react';
import styled from "styled-components";

// Helper: Inline SVGs for clean, dependency-free icons
const Icons = {
  email: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22,6 12,13 2,6" /></svg>,
  phone: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.05 12.05 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.05 12.05 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" /></svg>,
  location: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>,
  linkedin: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" /><rect x="2" y="9" width="4" height="13" /><circle cx="4" cy="4" r="2" /></svg>,
  github: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22" /></svg>,
  globe: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /></svg>
};

const parseMarkdown = (text) => {
  if (!text || typeof text !== 'string') return '';
  return text
    .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
    .replace(/\*(.+?)\*/g, '<em>$1</em>')
    .replace(/\n/g, '<br/>');
};

const StyledWrapper = styled.div`
  @media print {
    body { margin: 0; padding: 0; background-color: white !important; }
    @page { size: A4; margin: 0; }
    .resume { width: 100%; box-shadow: none !important; margin: 0 !important; }
    * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  }

  body { font-family: 'Arial', sans-serif; background-color: #f5f5f5; }

  .resume {
    width: 210mm;
    min-height: 297mm;
    background: white;
    margin: 20px auto;
    padding: 40px;
    box-shadow: 0 0 20px rgba(0, 0, 0, 0.1);
    color: #333;
  }

  /* HEADER */
  .header-container {
    display: flex;
    justify-content: space-between;
    align-items: flex-start;
    margin-bottom: 20px;
    border-bottom: 2px solid #333;
    padding-bottom: 20px;
  }

  .header-content { flex: 1; }

  .name {
    font-size: 26px;
    font-weight: 800;
    text-transform: uppercase;
    margin: 0 0 5px 0;
    color: #000;
    letter-spacing: 0.5px;
  }

  .job-title {
    font-size: 14px;
    color: #555;
    font-weight: bold;
    margin-bottom: 12px;
    text-transform: uppercase;
  }

  .contact-info {
    display: flex;
    flex-wrap: wrap;
    gap: 12px 20px;
    font-size: 9px;
    color: #444;
  }

  .contact-item {
    display: flex;
    align-items: center;
    gap: 5px;
  }

  .contact-item svg {
    width: 12px;
    height: 12px;
    color: #333;
  }

  .profile-photo {
    width: 80px;
    height: 80px;
    margin-left: 20px;
    border: 1px solid #ddd;
    overflow: hidden;
    flex-shrink: 0;
  }
  
  .profile-photo img {
    width: 100%;
    height: 100%;
    object-fit: cover;
  }

  /* SECTIONS */
  .section { margin-bottom: 20px; }

  .section-title {
    font-size: 11px;
    font-weight: 800;
    text-transform: uppercase;
    color: #000;
    margin-bottom: 10px;
    padding-bottom: 4px;
    border-bottom: 1px solid #ccc;
    letter-spacing: 1px;
  }

  .profile-text {
    font-size: 10px;
    line-height: 1.6;
    color: #444;
    text-align: justify;
  }

  /* EXPERIENCE & PROJECTS */
  .item-block { margin-bottom: 12px; }

  .item-header {
    display: flex;
    justify-content: space-between;
    align-items: baseline;
    margin-bottom: 2px;
  }

  .item-title {
    font-size: 11px;
    font-weight: bold;
    color: #000;
  }

  .item-date {
    font-size: 10px;
    font-weight: 600;
    color: #555;
  }

  .item-subtitle {
    font-size: 10px;
    color: #666;
    font-style: italic;
    margin-bottom: 4px;
    display: flex;
    justify-content: space-between;
  }

  .item-description {
    font-size: 10px;
    line-height: 1.45;
    color: #444;
  }

  .item-description ul { margin: 4px 0; padding-left: 15px; }
  .item-description li { margin-bottom: 3px; }

  /* COMPACT GRID FOR SKILLS/CERTS/LANGS */
  .bottom-grid {
    display: flex;
    gap: 30px;
  }
  
  .grid-column { flex: 1; }

  .skill-group { margin-bottom: 10px; }
  .skill-label { font-size: 10px; font-weight: bold; color: #333; margin-bottom: 4px; }
  .skill-list { font-size: 9px; color: #555; line-height: 1.4; }

  .compact-item {
    display: flex;
    justify-content: space-between;
    font-size: 9px;
    margin-bottom: 4px;
    color: #555;
    border-bottom: 1px dotted #eee;
    padding-bottom: 2px;
  }
  .compact-item:last-child { border-bottom: none; }
  .compact-left { font-weight: 600; color: #333; }
`;

export const T20 = ({ jsonData }) => {
  const contact = jsonData.contactInfo || {};

  // Work Experience
  const workExpList = jsonData.workExperience?.map((we, index) => (
    <div key={`work-${index}`} className="item-block">
      <div className="item-header">
        <div className="item-title">{we.jobTitle || 'Job Title'}</div>
        <div className="item-date">{we.WorkDuration || 'Dates'}</div>
      </div>
      <div className="item-subtitle">
        <span>{we.companyName || 'Company'}</span>
        <span>{we.Location}</span>
      </div>
      <div className="item-description" dangerouslySetInnerHTML={{ __html: parseMarkdown(we.keyAchievements || '') }} />
    </div>
  ));

  // Projects
  const projectList = jsonData.projects?.map((proj, index) => (
    <div key={`proj-${index}`} className="item-block">
      <div className="item-header">
        <div className="item-title">{proj.projectTitle || proj.name || 'Project Name'}</div>
        {proj.link && (
          <div className="item-date">
            <a href={proj.link} target="_blank" rel="noreferrer" style={{ color: '#444', textDecoration: 'none' }}>Link ↗</a>
          </div>
        )}
      </div>
      <div className="item-subtitle">
        <span>{proj.toolsTechUsed ? `Tech: ${proj.toolsTechUsed}` : ''}</span>
      </div>
      <div className="item-description" dangerouslySetInnerHTML={{ __html: parseMarkdown(proj.description || '') }} />
    </div>
  ));

  // Education
  const educationList = jsonData.education?.map((edu, index) => (
    <div key={`edu-${index}`} className="item-block">
      <div className="item-header">
        <div className="item-title">{edu.degreeName || 'Degree'}</div>
        <div className="item-date">{edu.graduationYear || 'Year'}</div>
      </div>
      <div className="item-subtitle">
        <span>{edu.institutionName || 'University'}</span>
        {edu.currentCGPA && <span>CGPA: {edu.currentCGPA}</span>}
      </div>
    </div>
  ));

  // Data Preparation
  const hardSkills = jsonData.skills?.hardSkills
    ? (Array.isArray(jsonData.skills.hardSkills) ? jsonData.skills.hardSkills : jsonData.skills.hardSkills.split(',').filter(Boolean))
    : [];
  const softSkills = jsonData.skills?.softSkills
    ? (Array.isArray(jsonData.skills.softSkills) ? jsonData.skills.softSkills : jsonData.skills.softSkills.split(',').filter(Boolean))
    : [];
  const certificates = jsonData.certificates || [];
  const languages = contact.Languages
    ? (Array.isArray(contact.Languages) ? contact.Languages : contact.Languages.split(',').filter(Boolean))
    : [];

  return (
    <StyledWrapper>
      <div className="resume" id="capture-content">

        {/* HEADER */}
        <div className="header-container">
          <div className="header-content">
            <div className="name">{contact.fullName || 'Your Name'}</div>
            <div className="job-title">{contact.jobTitle || 'Professional Title'}</div>

            <div className="contact-info">
              {contact.emailAddress && (
                <div className="contact-item">
                  {Icons.email} <span>{contact.emailAddress}</span>
                </div>
              )}
              {contact.phoneNumber && (
                <div className="contact-item">
                  {Icons.phone} <span>{contact.phoneNumber}</span>
                </div>
              )}
              {contact.Location && (
                <div className="contact-item">
                  {Icons.location} <span>{contact.Location}</span>
                </div>
              )}
              {contact.linkedin && (
                <div className="contact-item">
                  {Icons.linkedin} <span>{contact.linkedin.replace(/^https?:\/\/(www\.)?linkedin\.com\/in\//, '')}</span>
                </div>
              )}
              {contact.github && (
                <div className="contact-item">
                  {Icons.github} <span>{contact.github.replace(/^https?:\/\/(www\.)?github\.com\//, '')}</span>
                </div>
              )}
              {contact.portfolio && !contact.github && (
                <div className="contact-item">
                  {Icons.globe} <span>{contact.portfolio.replace(/^https?:\/\//, '')}</span>
                </div>
              )}
            </div>
          </div>

          {contact.profileImage && (
            <div className="profile-photo">
              <img src={contact.profileImage} alt="Profile" />
            </div>
          )}
        </div>

        {/* PROFILE */}
        {jsonData.Description?.UserDescription && (
          <div className="section">
            <div className="section-title">Professional Summary</div>
            <div className="profile-text" dangerouslySetInnerHTML={{ __html: parseMarkdown(jsonData.Description.UserDescription) }} />
          </div>
        )}

        {/* WORK EXPERIENCE */}
        {workExpList && workExpList.length > 0 && (
          <div className="section">
            <div className="section-title">Work Experience</div>
            {workExpList}
          </div>
        )}

        {/* PROJECTS */}
        {projectList && projectList.length > 0 && (
          <div className="section">
            <div className="section-title">Projects</div>
            {projectList}
          </div>
        )}

        {/* EDUCATION */}
        {educationList && educationList.length > 0 && (
          <div className="section">
            <div className="section-title">Education</div>
            {educationList}
          </div>
        )}

        {/* BOTTOM GRID (Skills, Certs, Languages) */}
        <div className="section bottom-grid">

          {/* Column 1: Skills */}
          {(hardSkills.length > 0 || softSkills.length > 0) && (
            <div className="grid-column">
              <div className="section-title">Skills</div>
              {hardSkills.length > 0 && (
                <div className="skill-group">
                  <div className="skill-label">Hard Skills</div>
                  <div className="skill-list">{hardSkills.map(s => s.trim()).join(', ')}</div>
                </div>
              )}
              {softSkills.length > 0 && (
                <div className="skill-group">
                  <div className="skill-label">Soft Skills</div>
                  <div className="skill-list">{softSkills.map(s => s.trim()).join(', ')}</div>
                </div>
              )}
            </div>
          )}

          {/* Column 2: Certifications & Languages */}
          {(certificates.length > 0 || languages.length > 0) && (
            <div className="grid-column">
              {certificates.length > 0 && (
                <>
                  <div className="section-title">Certifications</div>
                  {certificates.map((cert, i) => (
                    <div key={i} className="compact-item">
                      <span className="compact-left">{cert.certificateName}</span>
                      <span>{cert.year || ''}</span>
                    </div>
                  ))}
                  <div style={{ marginBottom: '15px' }}></div>
                </>
              )}

              {languages.length > 0 && (
                <>
                  <div className="section-title">Languages</div>
                  {languages.map((lang, i) => (
                    <div key={i} className="compact-item">
                      <span className="compact-left">{typeof lang === 'string' ? lang.trim() : (lang.name || lang)}</span>
                      <span>{typeof lang === 'object' ? (lang.level || '') : ''}</span>
                    </div>
                  ))}
                </>
              )}
            </div>
          )}
        </div>

      </div>
    </StyledWrapper>
  );
};

export const T20Css = `
@media print {
  body { margin: 0; padding: 0; background-color: white !important; }
  @page { size: A4 portrait; margin: 0; }
  .resume { 
    width: 210mm !important; 
    max-width: 210mm !important; 
    min-height: 297mm !important; 
    margin: 0 !important; 
    box-shadow: none !important; 
    background-color: white !important;
    padding: 40px !important;
  }
  * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
}

body { font-family: 'Arial', sans-serif; background-color: #f5f5f5; }

.resume {
  width: 210mm;
  min-height: 297mm;
  background: white;
  margin: 20px auto;
  padding: 40px;
  box-shadow: 0 0 20px rgba(0, 0, 0, 0.1);
  color: #333;
}

.header-container {
  display: flex;
  justify-content: space-between;
  align-items: flex-start;
  margin-bottom: 20px;
  border-bottom: 2px solid #333;
  padding-bottom: 20px;
}

.header-content { flex: 1; }

.name {
  font-size: 26px;
  font-weight: 800;
  text-transform: uppercase;
  margin: 0 0 5px 0;
  color: #000;
  letter-spacing: 0.5px;
}

.job-title {
  font-size: 14px;
  color: #555;
  font-weight: bold;
  margin-bottom: 12px;
  text-transform: uppercase;
}

.contact-info {
  display: flex;
  flex-wrap: wrap;
  gap: 12px 20px;
  font-size: 9px;
  color: #444;
}

.contact-item {
  display: flex;
  align-items: center;
  gap: 5px;
}

.contact-item svg {
  width: 12px;
  height: 12px;
  color: #333;
}

.profile-photo {
  width: 80px;
  height: 80px;
  margin-left: 20px;
  border: 1px solid #ddd;
  overflow: hidden;
  flex-shrink: 0;
}

.profile-photo img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

.section { margin-bottom: 20px; }

.section-title {
  font-size: 11px;
  font-weight: 800;
  text-transform: uppercase;
  color: #000;
  margin-bottom: 10px;
  padding-bottom: 4px;
  border-bottom: 1px solid #ccc;
  letter-spacing: 1px;
}

.profile-text {
  font-size: 10px;
  line-height: 1.6;
  color: #444;
  text-align: justify;
}

.item-block { margin-bottom: 12px; }

.item-header {
  display: flex;
  justify-content: space-between;
  align-items: baseline;
  margin-bottom: 2px;
}

.item-title {
  font-size: 11px;
  font-weight: bold;
  color: #000;
}

.item-date {
  font-size: 10px;
  font-weight: 600;
  color: #555;
}

.item-subtitle {
  font-size: 10px;
  color: #666;
  font-style: italic;
  margin-bottom: 4px;
  display: flex;
  justify-content: space-between;
}

.item-description {
  font-size: 10px;
  line-height: 1.45;
  color: #444;
}

.item-description ul { margin: 4px 0; padding-left: 15px; }
.item-description li { margin-bottom: 3px; }

.bottom-grid {
  display: flex;
  gap: 30px;
}

.grid-column { flex: 1; }

.skill-group { margin-bottom: 10px; }
.skill-label { font-size: 10px; font-weight: bold; color: #333; margin-bottom: 4px; }
.skill-list { font-size: 9px; color: #555; line-height: 1.4; }

.compact-item {
  display: flex;
  justify-content: space-between;
  font-size: 9px;
  margin-bottom: 4px;
  color: #555;
  border-bottom: 1px dotted #eee;
  padding-bottom: 2px;
}
.compact-item:last-child { border-bottom: none; }
.compact-left { font-weight: 600; color: #333; }
`;
