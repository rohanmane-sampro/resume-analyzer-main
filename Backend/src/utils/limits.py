...
def get_effective_limits(user, global_settings=None):
    ...
    Returns a dict: {'template_limit': int, 'resume_download_limit': int}
    """
    ...
    if user.get('has_custom_limits') is True:
        return {
            'template_limit': int(user.get('template_limit', 3)),
            'resume_download_limit': int(user.get('resume_download_limit', 2))
        }
    ...
    if user_type == 'knowledge_hub':
        ...
        if plan_data:
             return {
                'template_limit': int(plan_data.get('templates', 3)),
                'resume_download_limit': int(plan_data.get('downloads', 2))
            }
        
        # STRICT: If plan is not found in Admin Settings, return 0.
        # This complies with "Limits must only come from Admin".
        return {
            'template_limit': 0,
    ...
    # Retrieve Guest limits safely
    guest_limits = global_settings.get('guest_limits') or {}
